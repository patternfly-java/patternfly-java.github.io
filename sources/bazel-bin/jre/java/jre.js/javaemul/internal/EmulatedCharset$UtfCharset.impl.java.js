...
let $Overlay = goog.forwardDeclare('javaemul.internal.PrimitiveLists.Byte.$Overlay$impl');
...
class UtfCharset extends EmulatedCharset {
 ...
 m_decodeString__arrayOf_byte__int__int__boolean__arrayOf_char(/** Array<number> */ bytes, /** number */ ofs, /** number */ len, /** boolean */ throwOnInvalid) {
  ...
  chars = (/**@type {Array<number>}*/ ($Arrays.$castTo(ArrayHelper.m_setLength__java_lang_Object__int__java_lang_Object(chars, Math.imul(len, 2)), $char, 1)));
  let outIdx = 0;
  let count = 0;
  for (let i = 0; i < len; ) {
   let invalid = false;
   ...
   if (ch <= Character.f_MAX_VALUE__java_lang_Character && Character.m_isSurrogate__char__boolean(Primitives.m_narrowIntToChar__int__int(ch))) {
    invalid = true;
   }
  ...
  chars = (/**@type {Array<number>}*/ ($Arrays.$castTo(ArrayHelper.m_setLength__java_lang_Object__int__java_lang_Object(chars, outIdx), $char, 1)));
  return chars;
 }
 ...
 m_getBytes__arrayOf_char__int__int__boolean__arrayOf_byte(/** Array<number> */ buffer, /** number */ offset, /** number */ count, /** boolean */ throwOnInvalid) {
  ...
  let bytes = PrimitiveLists.m_createForByte__javaemul_internal_PrimitiveLists_Byte();
  for (let i = offset; i < n; ) {
   ...
   this.m_encodeUtf8__javaemul_internal_PrimitiveLists_Byte__int__boolean__void_$p_javaemul_internal_EmulatedCharset_UtfCharset(bytes, ch, throwOnInvalid);
  }
  return $Overlay.m_toArray__$devirt__javaemul_internal_PrimitiveLists_Byte__arrayOf_byte(bytes);
 }
 /** @nodts @return {number} */
 static m_getCodePointAt__arrayOf_char__int__boolean__int(/** Array<number> */ buffer, /** number */ pos, /** boolean */ throwOnInvalid) {
  ...
  let high = ch;
  let low = (pos + 1 | 0) < buffer.length ? buffer[pos + 1 | 0] : 255;
  if (!Character.m_isSurrogatePair__char__char__boolean(high, low)) {
   if (throwOnInvalid) {
    throw Exceptions.m_toJs__java_lang_Throwable__javaemul_internal_ThrowableUtils_JsObject(IllegalArgumentException.$create__java_lang_String('Invalid surrogate pair'));
   } else {
    return 63 /* '?' */;
   }
  }
 ...
 static m_getCodePointAt__java_lang_String__int__int__boolean__int(/** ?string */ str, /** number */ pos, /** number */ length, /** boolean */ throwOnInvalid) {
  ...
  let high = ch;
  let low = (pos + 1 | 0) < length ? j_l_String.m_charAt__java_lang_String__int__char(str, pos + 1 | 0) : 255;
  if (!Character.m_isSurrogatePair__char__char__boolean(high, low)) {
   if (throwOnInvalid) {
    throw Exceptions.m_toJs__java_lang_Throwable__javaemul_internal_ThrowableUtils_JsObject(IllegalArgumentException.$create__java_lang_String('Invalid surrogate pair'));
   } else {
 ...
 /** @nodts */
 m_encodeUtf8__javaemul_internal_PrimitiveLists_Byte__int__boolean__void_$p_javaemul_internal_EmulatedCharset_UtfCharset(/** Array */ bytes, /** number */ codePoint, /** boolean */ throwOnInvalid) {
  if (codePoint >= 1 << 26) {
   if (throwOnInvalid) {
    throw Exceptions.m_toJs__java_lang_Throwable__javaemul_internal_ThrowableUtils_JsObject(IllegalArgumentException.$create__java_lang_String('Character out of range: ' + codePoint));
   } else {
    codePoint = UtfCharset.f_REPLACEMENT_CHAR__javaemul_internal_EmulatedCharset_UtfCharset_;
   }
  }
  if (codePoint < 1 << 7) {
   bytes.push(Primitives.m_narrowIntToByte__int__int(codePoint & 127));
  } else if (codePoint < 1 << 11) {
   bytes.push(Primitives.m_narrowIntToByte__int__int(codePoint >> 6 & 31 | 192));
   bytes.push(Primitives.m_narrowIntToByte__int__int(codePoint & 63 | 128));
  } else if (codePoint < 1 << 16) {
   bytes.push(Primitives.m_narrowIntToByte__int__int(codePoint >> 12 & 15 | 224));
   bytes.push(Primitives.m_narrowIntToByte__int__int(codePoint >> 6 & 63 | 128));
   bytes.push(Primitives.m_narrowIntToByte__int__int(codePoint & 63 | 128));
  } else if (codePoint < 1 << 21) {
   bytes.push(Primitives.m_narrowIntToByte__int__int(codePoint >> 18 & 7 | 240));
   bytes.push(Primitives.m_narrowIntToByte__int__int(codePoint >> 12 & 63 | 128));
   bytes.push(Primitives.m_narrowIntToByte__int__int(codePoint >> 6 & 63 | 128));
   bytes.push(Primitives.m_narrowIntToByte__int__int(codePoint & 63 | 128));
  } else {
   bytes.push(Primitives.m_narrowIntToByte__int__int(codePoint >> 24 & 3 | 248));
   bytes.push(Primitives.m_narrowIntToByte__int__int(codePoint >> 18 & 63 | 128));
   bytes.push(Primitives.m_narrowIntToByte__int__int(codePoint >> 12 & 63 | 128));
   bytes.push(Primitives.m_narrowIntToByte__int__int(codePoint >> 6 & 63 | 128));
   bytes.push(Primitives.m_narrowIntToByte__int__int(codePoint & 63 | 128));
  }
 ...
 static $loadModules() {
  ...
  $Overlay = goog.module.get('javaemul.internal.PrimitiveLists.Byte.$Overlay$impl');
