...
class MathContext extends j_l_Object {
 ...
 $ctor__java_math_MathContext__java_lang_String__void(/** ?string */ val) {
  ...
  if ($Equality.$same(match, null) || match.length != 3) {
   throw Exceptions.m_toJs__java_lang_Throwable__javaemul_internal_ThrowableUtils_JsObject(IllegalArgumentException.$create__java_lang_String('bad string format'));
  }
  try {
   this.f_precision__java_math_MathContext_ = Integer.m_parseInt__java_lang_String__int(match.at(1));
   this.f_roundingMode__java_math_MathContext_ = RoundingMode.m_valueOf__java_lang_String__java_math_RoundingMode(match.at(2));
  } catch (__$jsexc) {
