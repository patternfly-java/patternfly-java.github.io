...
class Conversion extends j_l_Object {
 ...
 static m_bigInteger2String__java_math_BigInteger__int__java_lang_String(/** BigInteger */ val, /** number */ radix) {
  ...
  if (numberLength == 1) {
   ...
   let v = $LongUtils.bitAnd(Primitives.m_widenIntToLong__int__long(highDigit), $Long.fromBits(-1, 0) /* 4294967295 */);
   if (sign < 0) {
  ...
  let resLengthInChars = Primitives.m_narrowDoubleToInt__int__int(val.m_abs__java_math_BigInteger().m_bitLength__int() / bitsForRadixDigit + (sign < 0 ? 1 : 0)) + 1 | 0;
  ...
  if (radix != 16) {
   ...
   while (true) {
    ...
    do {
     $Arrays.$set(result, currentChar = currentChar - 1 | 0, Character.m_forDigit__int__int__char(Primitives.m_coerceDivision__int__int(resDigit % radix), radix));
    } while ((resDigit = Primitives.m_coerceDivision__int__int(resDigit / radix)) != 0 && currentChar != 0);
  ...
  if (sign == 0) {
   switch (scale) {
    ...
    case 6:
     return '0.000000';
    default:
     let result1 = StringBuilder.$create__();
     if (scale < 0) {
  ...
  if (numberLength == 1) {
   ...
   if (highDigit < 0) {
    ...
    do {
     ...
     $Arrays.$set(result, currentChar = currentChar - 1 | 0, Primitives.m_narrowIntToChar__int__int(48 + Primitives.m_narrowLongToInt__long__int($LongUtils.minus(prev, $LongUtils.times(v, $Long.fromInt(10)))) | 0));
    } while ($LongUtils.notEquals(v, $Long.fromInt(0)));
   } else {
    let v_1 = highDigit;
    do {
     let prev_1 = v_1;
     v_1 = Primitives.m_coerceDivision__int__int(v_1 / 10);
     $Arrays.$set(result, currentChar = currentChar - 1 | 0, Primitives.m_narrowIntToChar__int__int(48 + (prev_1 - Math.imul(v_1, 10)) | 0));
    } while (v_1 != 0);
   }
  } else {
   let temp = /**@type {!Array<number>}*/ ($Arrays.$create([numberLength], $int));
   ...
   BIG_LOOP: while (true) {
    ...
    for (let i1 = tempLen - 1 | 0; i1 >= 0; i1 = i1 - 1 | 0) {
     let temp1 = $LongUtils.plus($LongUtils.leftShift(result11, 32), $LongUtils.bitAnd(Primitives.m_widenIntToLong__int__long(temp[i1]), $Long.fromBits(-1, 0) /* 4294967295 */));
     let res = Conversion.m_divideLongByBillion__long__long(temp1);
     $Arrays.$set(temp, i1, Primitives.m_narrowLongToInt__long__int(res));
     result11 = Primitives.m_widenIntToLong__int__long(Primitives.m_narrowLongToInt__long__int($LongUtils.rightShiftSigned(res, 32)));
    }
    let resDigit = Primitives.m_narrowLongToInt__long__int(result11);
    let previous = currentChar;
    do {
     $Arrays.$set(result, currentChar = currentChar - 1 | 0, Primitives.m_narrowIntToChar__int__int(48 + Primitives.m_coerceDivision__int__int(resDigit % 10) | 0));
    } while ((resDigit = Primitives.m_coerceDivision__int__int(resDigit / 10)) != 0 && currentChar != 0);
 ...
 static m_toDecimalScaledString__long__int__java_lang_String(/** !$Long */ value, /** number */ scale) {
  ...
  do {
   let prev = v;
   v = $LongUtils.divide(v, $Long.fromInt(10));
   $Arrays.$set(result, currentChar = currentChar - 1 | 0, Primitives.m_narrowLongToChar__long__int($LongUtils.plus($Long.fromInt(48), $LongUtils.minus(prev, $LongUtils.times(v, $Long.fromInt(10))))));
  } while ($LongUtils.notEquals(v, $Long.fromInt(0)));
  let exponent = $LongUtils.minus($LongUtils.minus($LongUtils.minus(Primitives.m_widenIntToLong__int__long(resLengthInChars), Primitives.m_widenIntToLong__int__long(currentChar)), Primitives.m_widenIntToLong__int__long(scale)), $Long.fromInt(1));
  if (scale == 0) {
   if (negNumber) {
    $Arrays.$set(result, currentChar = currentChar - 1 | 0, 45 /* '-' */);
   }
  ...
  if (scale > 0 && $LongUtils.greaterEquals(exponent, $Long.fromInt(-6))) {
   if ($LongUtils.greaterEquals(exponent, $Long.fromInt(0))) {
    let insertPoint = currentChar + Primitives.m_narrowLongToInt__long__int(exponent) | 0;
   ...
   for (let j_1 = 2; $LongUtils.less(Primitives.m_widenIntToLong__int__long(j_1), $LongUtils.plus($LongUtils.negate(exponent), $Long.fromInt(1))); j_1 = j_1 + 1 | 0) {
    $Arrays.$set(result, currentChar = currentChar - 1 | 0, 48 /* '0' */);
   }
   $Arrays.$set(result, currentChar = currentChar - 1 | 0, 46 /* '.' */);
   $Arrays.$set(result, currentChar = currentChar - 1 | 0, 48 /* '0' */);
 ...
 static $clinit() {
  ...
  j_l_Object.$clinit();
  Conversion.$static_bigRadices__java_math_Conversion = (/**@type {!Array<number>}*/ ($Arrays.$stampType([-2147483648, 1162261467, 1073741824, 1220703125, 362797056, 1977326743, 1073741824, 387420489, 1000000000, 214358881, 429981696, 815730721, 1475789056, 170859375, 268435456, 410338673, 612220032, 893871739, 1280000000, 1801088541, 113379904, 148035889, 191102976, 244140625, 308915776, 387420489, 481890304, 594823321, 729000000, 887503681, 1073741824, 1291467969, 1544804416, 1838265625, 60466176], $int)));
  Conversion.$static_digitFitInInt__java_math_Conversion = (/**@type {!Array<number>}*/ ($Arrays.$stampType([-1, -1, 31, 19, 15, 13, 11, 11, 10, 9, 9, 8, 8, 8, 8, 7, 7, 7, 7, 7, 7, 7, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 5], $int)));
 }
