...
class MessageDigestSpi extends j_l_Object {
 ...
 m_engineDigest__arrayOf_byte__int__int__int(/** Array<number> */ buf, /** number */ offset, /** number */ len) {
  ...
  if (buf.length < (digest.length + offset | 0)) {
   throw Exceptions.m_toJs__java_lang_Throwable__javaemul_internal_ThrowableUtils_JsObject(DigestException.$create__java_lang_String('Insufficient buffer space for digest'));
  }
  if (len < digest.length) {
   throw Exceptions.m_toJs__java_lang_Throwable__javaemul_internal_ThrowableUtils_JsObject(DigestException.$create__java_lang_String('Length not large enough to hold digest'));
  }
