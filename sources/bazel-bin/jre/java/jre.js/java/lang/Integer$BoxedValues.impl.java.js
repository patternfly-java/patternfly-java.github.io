...
class BoxedValues extends j_l_Object {
 ...
 /** @nodts @nosideeffects @return {Integer} */
 static m_get__int__java_lang_Integer(/** number */ i) {
  BoxedValues.$clinit();
  return BoxedValues.f_boxedValues__java_lang_Integer_BoxedValues_[i + 128 | 0];
 }
 /** @nodts */
 static $clinit() {
  BoxedValues.$clinit = () =>{};
  ...
  j_l_Object.$clinit();
  let values = /**@type {!Array<Integer>}*/ ($Arrays.$create([256], Integer));
  for (let i = 0; i < 256; i = i + 1 | 0) {
   $Arrays.$set(values, i, Integer.$create__int(i - 128 | 0));
  }
  BoxedValues.f_boxedValues__java_lang_Integer_BoxedValues_ = values;
 }
