...
class Collections extends j_l_Object {
 ...
 static m_copy__java_util_List__java_util_List__void(/** List<?> */ dest, /** List<?> */ src) {
  ...
  if (src.size() > dest.size()) {
   throw Exceptions.m_toJs__java_lang_Throwable__javaemul_internal_ThrowableUtils_JsObject(IndexOutOfBoundsException.$create__java_lang_String('src does not fit in dest'));
  }
 ...
 static m_nCopies__int__java_lang_Object__java_util_List(/** number */ n, /** T */ o) {
  Collections.$clinit();
  let array = /**@type {!Array<T>}*/ (new Array(n));
  if (!$Equality.$same(o, null)) {
   Arrays.m_fill__arrayOf_java_lang_Object__java_lang_Object__void(array, o);
  }
  return /**@type {List<T>}*/ (Collections.m_unmodifiableList__java_util_List__java_util_List(/**@type {List<T>}*/ (Arrays.m_asList__arrayOf_java_lang_Object__java_util_List(array))));
 }
 /** @nodts @template T @return {boolean} */
 ...
 static m_reverse__java_util_List__void(/** List<?> */ l) {
  ...
  if (RandomAccess.$isInstance(l)) {
   ...
   let /** ListIterator */ head = l.m_listIterator__java_util_ListIterator();
   let /** ListIterator */ tail = l.m_listIterator__int__java_util_ListIterator(l.size());
 ...
 static m_rotate__java_util_List__int__void(/** List<?> */ lst, /** number */ dist) {
  ...
  let normdist = Primitives.m_coerceDivision__int__int(dist % size);
  if (normdist == 0) {
   return;
  }
  if (normdist < 0) {
  ...
  if (RandomAccess.$isInstance(lst)) {
   ...
   for (let i = 0; i < size; i = i + 1 | 0) {
    index = Primitives.m_coerceDivision__int__int((index + normdist | 0) % size);
 ...
 static m_shuffle__java_util_List__java_util_Random__void(/** List<?> */ list, /** Random */ rnd) {
  ...
  if (RandomAccess.$isInstance(list)) {
   ...
   let arr = list.m_toArray__arrayOf_java_lang_Object();
   ...
   let /** ListIterator */ it = list.m_listIterator__java_util_ListIterator();
 ...
 static m_internalListOf__arrayOf_java_lang_Object__java_util_List(/** Array<E> */ elements) {
  Collections.$clinit();
  if (InternalPreconditions.m_isApiChecked__boolean()) {
   for (let i = 0; i < elements.length; i = i + 1 | 0) {
    InternalPreconditions.m_checkNotNull__java_lang_Object__java_lang_Object(elements[i]);
   }
  }
  return /**@type {!UnmodifiableRandomAccessList<E>}*/ (UnmodifiableRandomAccessList.$create__java_util_List(elements.length == 0 ? /**@type {List<E>}*/ (Collections.m_emptyList__java_util_List()) : /**@type {List<E>}*/ (Arrays.m_asList__arrayOf_java_lang_Object__java_util_List(elements))));
 }
 /** @nodts @template E @return {Set<E>} */
 static m_internalSetOf__arrayOf_java_lang_Object__boolean__java_util_Set(/** Array<E> */ elements, /** boolean */ allowDuplicates) {
  Collections.$clinit();
  if (elements.length == 0) {
   return /**@type {Set<E>}*/ (Collections.m_unmodifiableSet__java_util_Set__java_util_Set(/**@type {Set<E>}*/ (Collections.m_emptySet__java_util_Set())));
  }
  let set = /**@type {!HashSet<E>}*/ (HashSet.$create__());
  for (let i = 0; i < elements.length; i = i + 1 | 0) {
   let added = set.add(InternalPreconditions.m_checkNotNull__java_lang_Object__java_lang_Object(elements[i]));
   if (!allowDuplicates) {
    InternalPreconditions.m_checkArgument__boolean__java_lang_Object__void(added, 'Duplicate element');
   }
  }
  return /**@type {Set<E>}*/ (Collections.m_unmodifiableSet__java_util_Set__java_util_Set(set));
 }
 /** @template K, V @return {Map<K, V>} */
 static internalMapOf(.../** ...* */ elements) {
  Collections.$clinit();
  if (elements.length == 0) {
   return /**@type {Map<K, V>}*/ (Collections.m_unmodifiableMap__java_util_Map__java_util_Map(/**@type {Map<K, V>}*/ (Collections.m_emptyMap__java_util_Map())));
  }
  let map = /**@type {!HashMap<K, V>}*/ (HashMap.$create__());
  for (let i = 0; i < elements.length; i = i + 2 | 0) {
   let old = map.put(/**@type {?K}*/ (InternalPreconditions.m_checkNotNull__java_lang_Object__java_lang_Object(elements[i])), /**@type {?V}*/ (InternalPreconditions.m_checkNotNull__java_lang_Object__java_lang_Object(elements[i + 1 | 0])));
   InternalPreconditions.m_checkArgument__boolean__java_lang_Object__void($Equality.$same(old, null), 'Duplicate element');
  }
  return /**@type {Map<K, V>}*/ (Collections.m_unmodifiableMap__java_util_Map__java_util_Map(map));
 }
 /** @nodts @template K, V @return {Map<K, V>} */
 static m_internalMapFromEntries__java_util_Collection__java_util_Map(/** Collection<?> */ entries) {
  Collections.$clinit();
  if (entries.isEmpty()) {
   return /**@type {Map<K, V>}*/ (Collections.m_unmodifiableMap__java_util_Map__java_util_Map(/**@type {Map<K, V>}*/ (Collections.m_emptyMap__java_util_Map())));
  }
  ...
  for (let $iterator = entries.m_iterator__java_util_Iterator(); $iterator.m_hasNext__boolean(); ) {
   let entry = /**@type {Entry<*, *>}*/ ($Casts.$to($iterator.m_next__java_lang_Object(), /**@type {Function}*/ (Entry)));
   {
    InternalPreconditions.m_checkNotNull__java_lang_Object__java_lang_Object(entry);
    let old = map.put(InternalPreconditions.m_checkNotNull__java_lang_Object__java_lang_Object(entry.getKey()), InternalPreconditions.m_checkNotNull__java_lang_Object__java_lang_Object(entry.getValue()));
    InternalPreconditions.m_checkArgument__boolean__java_lang_Object__void($Equality.$same(old, null), 'Duplicate element');
   }
  }
  return /**@type {Map<K, V>}*/ (Collections.m_unmodifiableMap__java_util_Map__java_util_Map(map));
