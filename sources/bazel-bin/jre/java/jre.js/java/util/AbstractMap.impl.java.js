...
class AbstractMap extends j_l_Object {
 ...
 /** @override @return {boolean} */
 containsValue(/** * */ value) {
  for (let $iterator = this.entrySet().m_iterator__java_util_Iterator(); $iterator.m_hasNext__boolean(); ) {
 ...
 equals(/** * */ obj) {
  ...
  let otherMap = /**@type {Map<*, *>}*/ ($Casts.$to(obj, /**@type {Function}*/ (Map)));
  if (this.size() != otherMap.size()) {
   return false;
  }
  for (let $iterator = otherMap.entrySet().m_iterator__java_util_Iterator(); $iterator.m_hasNext__boolean(); ) {
 ...
 hashCode() {
  return Collections.m_hashCode__java_lang_Iterable__int(this.entrySet());
 }
 /** @override @return {boolean} */
 isEmpty() {
  return this.size() == 0;
 ...
 putAll(/** Map<?, ?> */ map) {
  ...
  for (let $iterator = map.entrySet().m_iterator__java_util_Iterator(); $iterator.m_hasNext__boolean(); ) {
   let e = /**@type {Entry<*, *>}*/ ($Casts.$to($iterator.m_next__java_lang_Object(), /**@type {Function}*/ (Entry)));
   {
 ...
 m_toString__java_util_Map_Entry__java_lang_String_$p_java_util_AbstractMap(/** Entry<K, V> */ entry) {
  return j_l_String.m_valueOf__java_lang_Object__java_lang_String(this.m_toString__java_lang_Object__java_lang_String_$p_java_util_AbstractMap(entry.getKey())) + '=' + j_l_String.m_valueOf__java_lang_Object__java_lang_String(this.m_toString__java_lang_Object__java_lang_String_$p_java_util_AbstractMap(entry.getValue()));
 }
 /** @nodts @return {?string} */
 ...
 static m_getEntryValueOrNull__java_util_Map_Entry__java_lang_Object(/** Entry<K_1, V_1> */ entry) {
  AbstractMap.$clinit();
  return $Equality.$same(entry, null) ? null : entry.getValue();
 }
 ...
 m_implFindEntry__java_lang_Object__boolean__java_util_Map_Entry_$p_java_util_AbstractMap(/** * */ key, /** boolean */ remove) {
  for (let iter = this.entrySet().m_iterator__java_util_Iterator(); iter.m_hasNext__boolean(); ) {
   let entry = /**@type {Entry<K, V>}*/ ($Casts.$to(iter.m_next__java_lang_Object(), /**@type {Function}*/ (Entry)));
   let k = entry.getKey();
   if (Objects.m_equals__java_lang_Object__java_lang_Object__boolean(key, k)) {
    if (remove) {
     entry = (/**@type {!SimpleEntry<K, V>}*/ (SimpleEntry.$create__java_lang_Object__java_lang_Object(entry.getKey(), entry.getValue())));
     iter.m_remove__void();
    }
    return entry;
   }
 ...
 /** @abstract @override @return {Set<Entry<K, V>>} */
 entrySet() {}
