...
class EnumMap extends AbstractMap {
 ...
 containsKey(/** * */ key) {
  return !$Equality.$same(this.m_getEntry__java_lang_Object__java_util_Map_Entry_$p_java_util_EnumMap(key), null);
 }
 /** @override @return {boolean} */
 containsValue(/** * */ value) {
 ...
 entrySet() {
  return this.f_map__java_util_EnumMap_.entrySet();
 }
 /** @override @return {V} */
 get(/** * */ key) {
  let entry = this.m_getEntry__java_lang_Object__java_util_Map_Entry_$p_java_util_EnumMap(key);
 ...
 remove(/** * */ key) {
  let entry = this.m_getEntry__java_lang_Object__java_util_Map_Entry_$p_java_util_EnumMap(key);
  return !$Equality.$same(entry, null) ? this.f_map__java_util_EnumMap_.remove(key) : null;
 }
 ...
 m_getEntry__java_lang_Object__java_util_Map_Entry_$p_java_util_EnumMap(/** * */ key) {
  if (!Enum.$isInstance(key)) {
   return null;
  }
  let entry = this.f_map__java_util_EnumMap_.m_findByObject__java_lang_Object__java_util_TreeMap_Node_$pp_java_util(key);
  return !$Equality.$same(entry, null) && (/**@type {!Enum<K>}*/ ($Casts.$to(entry.getKey(), Enum))).equals(key) ? entry : null;
 }
 //Bridge method.
