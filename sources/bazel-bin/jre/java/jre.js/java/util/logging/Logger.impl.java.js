...
class Logger extends j_l_Object {
 ...
 /** @nodts @nosideeffects @return {Logger} */
 ...
 m_addHandler__java_util_logging_Handler__void(/** Handler */ handler) {
  ...
  Logger.f_handlersAdded__java_util_logging_Logger_ = true;
 ...
 m_isLoggable__java_util_logging_Level__boolean(/** Level */ messageLevel) {
  if (!Logger.f_handlersAdded__java_util_logging_Logger_) {
   return false;
  }
  if (Logger.f_ALL_ENABLED__java_util_logging_Logger_) {
 ...
 m_actuallyLog__java_util_logging_LogRecord__void_$p_java_util_logging_Logger(/** LogRecord */ record) {
  let logger = this;
  while (!$Equality.$same(logger, null)) {
   for (let $array = logger.m_getHandlers__arrayOf_java_util_logging_Handler(), $index = 0; $index < $array.length; $index++) {
 ...
 static $clinit() {
  ...
  if (!$Equality.$same(level, 'ALL') && !$Equality.$same(level, 'INFO') && !$Equality.$same(level, 'WARNING') && !$Equality.$same(level, 'SEVERE') && !$Equality.$same(level, 'OFF')) {
   throw Exceptions.m_toJs__java_lang_Throwable__javaemul_internal_ThrowableUtils_JsObject(AssertionError.$create__java_lang_Object('Undefined value for jre.logging.logLevel: \'' + j_l_String.m_valueOf__java_lang_Object__java_lang_String(level) + '\''));
  }
...
/**@type {boolean} @nodts*/
Logger.f_handlersAdded__java_util_logging_Logger_ = false;
