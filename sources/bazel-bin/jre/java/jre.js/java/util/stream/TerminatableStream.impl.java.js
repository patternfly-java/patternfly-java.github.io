...
class TerminatableStream extends j_l_Object {
 ...
 m_throwIfTerminated__void_$pp_java_util_stream() {
  if (!$Equality.$same(this.f_root__java_util_stream_TerminatableStream_, null)) {
   ...
   throw Exceptions.m_toJs__java_lang_Throwable__javaemul_internal_ThrowableUtils_JsObject(IllegalStateException.$create__java_lang_String('Stream already terminated, can\'t be modified or used'));
  }
 }
 /** @nodts */
 ...
 m_runClosers__void_$p_java_util_stream_TerminatableStream() {
  ...
  this.f_onClose__java_util_stream_TerminatableStream_.m_forEach__java_util_function_Consumer__void(Consumer.$adapt((/** Runnable */ runnable) =>{
   let runnable_1 = /**@type {Runnable}*/ ($Casts.$to(runnable, /**@type {Function}*/ (Runnable)));
   try {
  ...
  if (!throwables.isEmpty()) {
   ...
   if (RuntimeException.$isInstance(e_1)) {
    throw Exceptions.m_toJs__java_lang_Throwable__javaemul_internal_ThrowableUtils_JsObject(/**@type {RuntimeException}*/ ($Casts.$to(e_1, RuntimeException)));
   }
   if (Error.$isInstance(e_1)) {
    throw Exceptions.m_toJs__java_lang_Throwable__javaemul_internal_ThrowableUtils_JsObject(/**@type {Error}*/ ($Casts.$to(e_1, Error)));
   }
   Asserts.$assertWithMessage__boolean__java_lang_Object__void(false, 'Couldn\'t have caught this exception from a Runnable! ' + j_l_String.m_valueOf__java_lang_Object__java_lang_String(e_1));
  }
 }
 /** @private @nodts */
