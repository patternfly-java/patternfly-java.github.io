...
class Partition extends AbstractList {
 ...
 static m_divide__int__int__int(/** number */ p, /** number */ q) {
  if (q == 0) {
   throw Exceptions.m_toJs__java_lang_Throwable__javaemul_internal_ThrowableUtils_JsObject(ArithmeticException.$create__java_lang_String('/ by zero'));
  }
  let div = Primitives.m_coerceDivision__int__int(p / q);
 ...
 m_checkElementIndex__int__int__void_$p_org_patternfly_dataprovider_DataProvider_Partition(/** number */ index, /** number */ size) {
  if (index < 0) {
   throw Exceptions.m_toJs__java_lang_Throwable__javaemul_internal_ThrowableUtils_JsObject(IndexOutOfBoundsException.$create__java_lang_String('Index ' + index + ' must not be negative'));
  } else if (index >= size) {
   throw Exceptions.m_toJs__java_lang_Throwable__javaemul_internal_ThrowableUtils_JsObject(IndexOutOfBoundsException.$create__java_lang_String('Index (' + index + ') must be less than size (' + size + ')'));
  }
 }
 /** @nodts */
 static $clinit() {
