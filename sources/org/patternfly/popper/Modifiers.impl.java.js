...
class Modifiers {
 ...
 static m_widths__org_patternfly_popper_Modifier() {
  ...
  modifier.requires = (/**@type {!Array<?string>}*/ ($Arrays.$stampType(['computeStyles'], j_l_String)));
 ...
 static m_placement__org_patternfly_popper_Modifier() {
  ...
  modifier.fn = (/** Object */ args) =>{
   if (!$Equality.$same(args.state, null) && !$Equality.$same(args.state.elements, null)) {
    let placement = PopperPlacement.m_of__java_lang_String__org_patternfly_popper_PopperPlacement(args.state.placement);
    let popper = args.state.elements.popper;
    if (!$Equality.$same(placement, null) && !$Equality.$same(placement, PopperPlacement.f_auto__org_patternfly_popper_PopperPlacement) && !$Equality.$same(popper, null)) {
     popper.classList.remove(...PopperPlacement.f_modifiers__org_patternfly_popper_PopperPlacement);
     popper.classList.add(placement.m_modifier__java_lang_String());
    }
   }
  };
  return modifier;
 ...
 static m_offset__int__org_patternfly_popper_Modifier(/** number */ offset) {
  ...
  modifier.options.offset = (/**@type {!Array<number>}*/ ($Arrays.$stampType([0, offset], $int)));
  return modifier;
 }
 /** @nodts @return {Object} */
...
$Util.$setClassMetadataForInterface(/**@type {Function}*/ (Modifiers), 'org.patternfly.popper.Modifiers');
